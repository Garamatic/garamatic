...
async function main() {
  ...
  await runner.describe('Agent Chat', async () => {
    await runner.it('should handle agent chat request', async () => {
      try {
        const response = await fetchWithTimeout(`${SERVICES.agenticService}/agent/chat`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
  ...
  await runner.describe('Validation & Error Handling', async () => {
    await runner.it('should reject ticket creation without title', async () => {
      try {
        ...
        runner.assertTrue(
          ...
          'Should reject missing title with 400/422'
        );
      } catch {
    ...
    await runner.it('should reject invalid email', async () => {
      try {
        ...
        runner.assertTrue(
          response.status === 400 || response.status === 422 || response.status === 503,
          'Should reject invalid email with 400/422'
        );
      } catch {
    ...
    await runner.it('should reject negative invoice amount', async () => {
      try {
        const response = await fetchWithTimeout(`${SERVICES.agenticService}/invoices/TEST-001`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            amount: -50.00,
