...
This will:
1. Start all services (ticket-masala, gatekeeper, mailing-service, agentic-service, odoo-integration, showcase, rabbitmq, mailhog, portal, odoo)
...
Checks health of all services, validates API endpoints, confirms RabbitMQ exchange, and reports demo data counts.
...
- **Live Service Topology** — Real-time health status of all services, polled every 5 seconds
...
├── showcase/                   # Architecture dashboard + citizen portal
│   ├── index.html              # Interactive architecture dashboard
│   └── portal/                 # Citizen portal frontend (React + Vite)
...
| Portal | http://localhost:8093 | Citizen portal frontend |
...
5. **Citizen portal** form submissions → create tickets via ticket-masala API
